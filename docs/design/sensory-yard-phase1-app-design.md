...
- Zone cards match the taxonomy (§5.2 — six ids now, not five) and the plan-generation JSON output 1:1 — each JSON zone object renders as one card, so the frontend is a thin, dumb renderer over the schema. Switching intake formats, and later switching from mocked to real generation, never changed this data contract.
