...
- Zone cards match source-spec §4.1 taxonomy and §4.2 JSON output structure 1:1 — each JSON zone object renders as one card, so the frontend is a thin, dumb renderer over the schema. The plan-generation call in §5.2 returns this same schema, so switching intake formats didn't change the report's data contract.
