...
- **Vector illustration, not AI image generation — a deliberate call.** Real image generation (e.g. a diffusion model) was considered and rejected for this: it's slow (seconds to tens of seconds per report, against a "5 minute" landing-page promise), costs real money per report on top of the plan-generation call — directly undercutting the usage-limit system built specifically to bound LLM spend (§9) — and image models are unreliable at accurate spatial layout and legible labels, which risks a report that visibly mislabels or misplaces a zone. Instead, everything is procedurally generated inline SVG: organic "blob" shapes and a wobbled yard boundary from a small deterministic path-generation library, plus a set of hand-coded per-zone-type doodle components (tree, flowers, sprouts, pebbles, motion swoosh, grass tufts). It's instant, free, crisp at any zoom, and fully deterministic — every shape is seeded from the report id and zone id (never `Math.random()`), so the same report always renders the same sketch and server/client hydration stays consistent. If AI-generated imagery is revisited later, it's an additive option behind this same zone data contract, not a replacement for it.
