...
- **Zip-code climate inference is an approximation, not a lookup.** The plan prompt asks the model to infer regional climate/water-cost from the zip code using its own general geographic knowledge (§5.2) — reasonable for well-known regions but not guaranteed accurate everywhere, and there's no real climate-zone data source behind it. If plant/water suggestions turn out unreliable in practice, the fix is a real zip→climate-zone or zip→USDA-hardiness-zone lookup (a small, well-known dataset) feeding an explicit value into the prompt instead of relying on inference.
